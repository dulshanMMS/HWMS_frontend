...
const ReviewsAndRatings = () => {
  ...
  const [unfilteredFeedbacks, setUnfilteredFeedbacks] = useState([]); // New state for unfiltered data
  ...
  const fetchFeedback = async () => {
    try {
      ...
      // Fetch unfiltered data (no params)
      const unfilteredResponse = await api.get('/api/ratings/feedback');
      setUnfilteredFeedbacks(unfilteredResponse.data);
  ...
  // Calculate statistics using unfilteredFeedbacks
  const totalFeedback = unfilteredFeedbacks.length;
  const averageRating = unfilteredFeedbacks.length
    ? (unfilteredFeedbacks.reduce((sum, fb) => sum + fb.rating, 0) / unfilteredFeedbacks.length).toFixed(1)
    : 0;
