    ...
    try {
      const response = await fetch('/api/notifications/preferences', {
        ...
        body: JSON.stringify({
          ...preferences,
          bookingReminder: { ...preferences.bookingReminder, inApp: true },
          adminAnnouncements: { ...preferences.adminAnnouncements, inApp: true }
        })
      });
      const data = await response.json();
      setPreferences(data);
                    ...
                    <div className="space-y-3">
                      ...
                      <label className="flex items-start space-x-3">
                        <input
                          type="checkbox"
                          checked={preferences[key].inApp}
                          onChange={(e) => key !== 'bookingReminder' && key !== 'adminAnnouncements' && handleChange(key, 'inApp', e.target.checked)}
                          disabled={key === 'bookingReminder' || key === 'adminAnnouncements'}
                          className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2 mt-1 disabled:opacity-50"
                        />
                        <div>
                          <div className="font-medium text-gray-900">In-App</div>
                          <div className="text-sm text-gray-500">
                            {key === 'bookingReminder' || key === 'adminAnnouncements'
